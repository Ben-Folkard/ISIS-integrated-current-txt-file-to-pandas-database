...
class Test_data_Processing(unittest.TestCase):
    ...
    def test_basic_single_date_get_integrated_current(self):
        """
        Tests to see whether the a succesful current is gathered for a known
        valid date
        """
    ...
    def test_basic_dates_get_integrated_current(self):
        """
        Tests to see whether succesful currents are gathered for a known valid
        range of dates
        """
    ...
    def test_summing_dates_get_integrated_current(self):
        """
        Tests to see whether succesful currents are gathered and summed for a
        known valid range of dates
        """
        integrated_currents = get_integrated_current(
            self.data, self.start_date, self.end_date
        )
        integrated_current_sum = get_integrated_current(
            self.data, self.start_date, self.end_date, is_summed=True
        )
    ...
    def test_is_beam_on(self):
        """
        Tests to see whether the code is properly registering the beam being
        on or off on known days where it is either on or off
        """
    ...
    def test_condensed_data_size(self):
        ...
        weekly = get_integrated_current(
            self.data, self.start_date, self.end_date, frequency="Weekly"
        )
        monthly = get_integrated_current(
            self.data, self.start_date, self.end_date, frequency="Monthly"
        )
    ...
    def test_summing_basic_get_num_protons(self):
        ...
        """
        num_protons_per_day = (
            get_num_protons(self.data, self.start_date, self.end_date) / 1e17
        )
        total_num_protons = (
            get_num_protons(self.data, self.start_date, self.end_date, is_summed=True)
            / 1e17
        )
    ...
    def test_condensed_proton_number(self):
        ...
        weekly = get_num_protons(
            self.data, self.start_date, self.end_date, frequency="Weekly"
        )
        monthly = get_num_protons(
            self.data, self.start_date, self.end_date, frequency="Monthly"
        )
