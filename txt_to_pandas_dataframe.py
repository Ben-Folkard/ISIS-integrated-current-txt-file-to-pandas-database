...
#Unit testing in seperate file
#Black or Flake8 tools pep8
...
def find_decreasing_value(data, column):
    ...
    for i, value in enumerate(data[column].values):
        if value<prev_value:
            ...
            print(f"{pd.to_datetime(data['Date'].values[i]).strftime('%d-%b-%Y')} ({column})")  
        prev_value = value
    return found
...
def data_integrity_check(data):
    ...
    """
    sorted_checks = ["Modified Julian day no.",
    ...
    for target in ("Sync", "TS-1", "TS-2"):
        assert (isclose(data[f"Milliamp-hours ({target})"].cumsum(), data[f"Cumulative milliamp-hours ({target})"], atol=0.1)).all()

    # Checks that the Date column doesn't have any duplicates
    assert not data["Modified Julian day no."].duplicated().any(), "Duplicated dates found"
...
def get_sliced_data(data, start_date, end_date, frequency="Daily", is_averaged=False):
    ...
    match frequency:
        ...
        case "Weekly":
            if is_averaged:
                used_data = data.iloc[start_index:end_index+1].resample("W", on="Date").mean(numeric_only=True)
            else:
                used_data = data.iloc[start_index:end_index+1].resample("W", on="Date").sum(numeric_only=True)
        case "Monthly":
            if is_averaged:
                used_data = data.iloc[start_index:end_index+1].resample("M", on="Date").mean(numeric_only=True)
            else:
                used_data = data.iloc[start_index:end_index+1].resample("M", on="Date").sum(numeric_only=True)
        case default:
...
def get_integrated_current(data, start_date, end_date=None, target="TS-1", valid_targets=("Sync", "TS-1", "TS-2"),
    ...
    if end_date == None:
        ...
        used_data = get_sliced_data(data, start_date, end_date, frequency=frequency, is_averaged=is_averaged)
...
def plot_integrated_current(data, start_date, end_date, target="TS-1", valid_targets=("Sync", "TS-1", "TS-2"), frequency="Daily",
                            is_averaged=False, is_high_resolution=False, is_shown=False, is_saved=True, date_format="%d-%b-%Y",
                            file_name=None):
    """
    ...
    if is_averaged and frequency=="Daily":
        print("Can't extract average daily data\n(minimum frequency for averaging = Weekly)")
        return None
...
def get_num_protons(data, start_date, end_date=None, target="TS-1", valid_targets=("Sync", "TS-1", "TS-2"),
                           frequency="Daily", is_averaged=False, is_summed=False, per_second=False):
    """
    Returns a list of the number of protons per specified amount of time (need to double check)
    """
    proton_charge      = 1.6e-19
    integrated_current = get_integrated_current(data, start_date, end_date,
                                                      target=target, valid_targets=valid_targets,
                                                      frequency=frequency, is_averaged=is_averaged,
                                                      is_summed=is_summed)
...
def get_average_power(data, start_date, end_date=None, target="TS-1", valid_targets=("Sync", "TS-1", "TS-2"),
    ...
    # Default: mAh -> A
    integrated_current  = current_unit_factor * get_integrated_current(data, start_date, end_date=end_date,
                                                                       target=target, valid_targets=valid_targets,
                                                                       frequency=frequency, is_averaged=is_averaged,
                                                                       is_summed=is_summed)
...
if __name__ == "__main__":
    ...
    plot_integrated_current(data, start_date, end_date, frequency="Weekly", is_averaged=True)
    plot_integrated_current(data, start_date, end_date, frequency="Monthly", is_averaged=True)
    print(f"{get_num_protons(data, start_date, is_summed=True):e}")
    print(get_num_protons(data, start_date, end_date, is_summed=True))
    print(get_num_protons(data, start_date, end_date, frequency="Weekly", is_averaged=True))
